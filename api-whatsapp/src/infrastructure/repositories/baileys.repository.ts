    ...
    try {
      ...
      socket.ev.on("connection.update", (update) => {
        ...
        if (connection === "close") {
          ...
          if (statusCode === 405) {
            console.log(`[${companyId}] Critical error (405). Cleaning session to force fresh start...`);
            try {
              // MySQL handle cleanup via table row deletion if needed, but for now we keep history or implement a delete method in auth adapter.
              // For now, we just clear local memory session.
              this.sessions.delete(companyId);

              // Also clean QR file if exists
              if (fs.existsSync(this.getQrFile(companyId))) fs.unlinkSync(this.getQrFile(companyId));

              // Reinitiate after cleanup
              setTimeout(() => this.startSession(companyId), 2000);
              return;
            } catch (cleanErr) {
